...
export async function updateMemberProfileAction(
  ...
  const raw = {
    ...
    avatar_data_url: String(formData.get("avatar_data_url") || "").trim() || undefined,
  };
  ...
  if (data.avatar_data_url) {
    ...
    if (!avatarUrl) {
      return { error: "Could not upload photo. Use JPG, PNG, or WebP under 5MB." };
    }
  }
  ...
  if (app?.id) {
    ...
    await admin.from("membership_applications").update(appPatch).eq("id", app.id);
  }
